...
public class StompChatController {
    ...
    public void enter(ChatMessageDto message) {
        ...
        if (existingRoom == null) {
            ...
            message = new ChatMessageDto(newRoom.getId(), message.senderId(), message.senderId() + "님이 채팅방에 참여하였습니다.");
        } else {
            // 이미 존재하는 채팅방에 입장 메시지를 전송
            message = new ChatMessageDto(existingRoom.getId(), message.senderId(), message.getSenderId() + "님이 채팅방에 참여하였습니다.");
        }
